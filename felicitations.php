...
$felicitationEvents = array_filter($events, function($e) use ($felicitationIds) {
    return in_array($e['id'], $felicitationIds);
});

// Sort by date (latest first)
usort($felicitationEvents, function($a, $b) {
...
if ($eventId) {
    foreach ($felicitationEvents as $e) {
        if ($e['id'] === $eventId) {
            $currentEvent = $e;
            break;
        }
...
<head>
    ...
    <title><?= $currentEvent ? htmlspecialchars($currentEvent['title']) : 'Felicitations & Awards' ?> - Dr. Vijay Anand Reddy</title>
...
<body class="bg-gray-50 flex flex-col min-h-screen">
        ...
        <div class="bg-white border-b border-gray-200 mt-28">
            <div class="max-w-7xl mx-auto px-6 py-4">
                <nav class="flex items-center gap-2 text-sm text-gray-600">
                    ...
                    <span>/</span>
                    <a href="felicitations.php" class="hover:text-medical-blue transition">Felicitations</a>
                    <span>/</span>
                        ...
                        <div class="flex justify-center mb-8">
                            <div class="inline-flex rounded-lg bg-gray-100 p-1">
                                <?php if (!empty($printPhotos)): ?>
                                    <button onclick="switchTab('print')" id="tab-btn-print" class="px-8 py-3 rounded-lg font-semibold transition-all duration-200 bg-medical-blue text-white shadow-md">Print Media</button>
                                <?php endif; ?>
                                <?php if (!empty($digitalPhotos)): ?>
                                    <button onclick="switchTab('digital')" id="tab-btn-digital" class="px-8 py-3 rounded-lg font-semibold transition-all duration-200 text-gray-600 hover:text-medical-dark">Digital Media</button>
                                <?php endif; ?>
                            </div>
                        </div>
    ...
    <?php else: ?>
        ...
        <!-- Hero Section -->
        <section class="pb-10 pt-44 text-center bg-gradient-to-br from-medical-blue via-purple-600 to-medical-blue relative overflow-hidden">
            ...
            <div class="relative z-10 max-w-4xl mx-auto px-6">
                ...
                <p class="text-blue-100 text-lg max-w-2xl mx-auto">
                    Celebrating milestones, honors, and community engagements
                </p>
            </div>
        </section>
        ...
        <div class="max-w-6xl mt-10 mx-auto px-6 mb-20 flex-grow">
            ...
            <?php else: ?>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-8">
                     <?php foreach ($felicitationEvents as $event): ?>
                        <div class="bg-white rounded-xl shadow-md hover:shadow-xl cursor-pointer transition transform hover:-translate-y-1 flex flex-col overflow-hidden" onclick="window.location.href='felicitations.php?id=<?= $event['id'] ?>'">
                            <div class="h-56 w-full relative">
                                <?php if (!empty($event['banners'])): ?>
                                    <img src="<?= htmlspecialchars($event['banners'][0]) ?>" alt="<?= htmlspecialchars($event['title']) ?>" class="h-full w-full object-cover" loading="lazy" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
                                ...
                                <?php if (!empty($event['date'])): ?>
                                    <div class="absolute top-4 right-4 bg-white/90 backdrop-blur px-4 py-2 rounded-full shadow-lg">
                                        <span class="text-medical-dark font-semibold text-sm">
                            ...
                            <div class="p-6 flex flex-col flex-1">
                                <h3 class="font-bold text-xl mb-3 text-medical-dark line-clamp-2">
                                ...
                                <button class="bg-medical-blue text-white font-medium px-6 py-3 rounded-lg hover:bg-purple-600 transition self-start flex items-center gap-2">
    ...
    <div id="photo-modal" class="fixed inset-0 z-50 bg-black/95 flex items-center justify-center p-4 backdrop-blur-sm hidden" onclick="closePhotoModal()">
        <div class="relative max-w-7xl w-full flex flex-col items-center" onclick="event.stopPropagation()">
            ...
            <div class="absolute bottom-4 left-1/2 -translate-x-1/2 bg-black/70 text-white px-4 py-2 rounded-full text-sm font-medium" id="photo-counter">1 / 1</div>
        </div>
    </div>
    ...
    <script>
        ...
        function nextBanner() { if (totalBanners > 1) updateBanner((currentBannerIdx + 1) % totalBanners); }
        function prevBanner() { if (totalBanners > 1) updateBanner((currentBannerIdx - 1 + totalBanners) % totalBanners); }
        function setBanner(idx) { updateBanner(idx); }

        // Tab Logic
        const galleryData = <?= json_encode($galleryData) ?>;
        let activeTab = 'simple';
        if (galleryData.hasTabs) { activeTab = 'print'; }
        ...
        function switchTab(tab) {
            ...
            if (tab === 'print') {
                if(btnPrint) { btnPrint.classList.add('bg-medical-blue', 'text-white', 'shadow-md'); btnPrint.classList.remove('text-gray-600', 'hover:text-medical-dark'); }
                if(btnDigital) { btnDigital.classList.remove('bg-medical-blue', 'text-white', 'shadow-md'); btnDigital.classList.add('text-gray-600', 'hover:text-medical-dark'); }
                ...
                if(btnPrint) { btnPrint.classList.remove('bg-medical-blue', 'text-white', 'shadow-md'); btnPrint.classList.add('text-gray-600', 'hover:text-medical-dark'); }
                if(btnDigital) { btnDigital.classList.add('bg-medical-blue', 'text-white', 'shadow-md'); btnDigital.classList.remove('text-gray-600', 'hover:text-medical-dark'); }
        ...
        function closePhotoModal() { modal.classList.add('hidden'); }
        ...
        function nextPhoto() { if (currentPhotos.length > 0) { currentPhotoIdx = (currentPhotoIdx + 1) % currentPhotos.length; updateModalImage(); } }
        function prevPhoto() { if (currentPhotos.length > 0) { currentPhotoIdx = (currentPhotoIdx - 1 + currentPhotos.length) % currentPhotos.length; updateModalImage(); } }
    </script>
